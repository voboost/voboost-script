...
const WEATHER_CONFIG_PATH = "/data/local/tmp/test/weather_config.json";
...
function getDailyExtremeTimes(dailyMap) {
    ...
    Object.keys(dailyMap).forEach(dateStr => {
        ...
        console.log(`[+] ${dateStr}: день=${extremes[dateStr].day.time} (${extremes[dateStr].day.temp}°C), ночь=${extremes[dateStr].night.time} (${extremes[dateStr].night.temp}°C)`);
    });
    return extremes;
}
