...
// Agent config lives under the root-owned voboost zone (/data/voboost,
// mode 0700, root:root) instead of /data/local/tmp (mode 1777, world-
// writable). The previous /data/local/tmp/voboost/ location allowed any
// app on the device to rewrite agent config at runtime. The daemon
// (voboost-inject) provisions these files under /data/voboost/agents/config/.
// The RPC `parameters.config` path (see getConfig) remains the preferred
// production delivery channel; these file paths are the on-disk fallback.
export const LANGUAGE_CONFIG_PATH = '/data/voboost/agents/config/language-config.json';
export const PHONE_NUM_CONFIG_PATH = '/data/voboost/agents/config/phone-num-config.json';
export const APP_CONFIG_PATH = '/data/voboost/agents/config/apps-config.json';
export const WEATHER_CONFIG_PATH = '/data/voboost/agents/config/weather-config.json';
export const APP_VIEWPORT_CONFIG_PATH = '/data/voboost/agents/config/apps-viewport-config.json';
export const KEYBOARD_TEMPLATE_PATH = '/data/voboost/agents/config/skb-qwerty-ru-no-voice.json';
export const KEYBOARD_RU_CONFIG_PATH = '/data/voboost/agents/config/keyboard-ru-config.json';
export const KEYBOARD_LOCK_EN_CONFIG_PATH =
    '/data/voboost/agents/config/keyboard-lock-en-config.json';
export const MEDIA_SOURCE_CONFIG_PATH = '/data/voboost/agents/config/media-source-config.json';
...
// Cap on the Base64-encoded icon string length before decode. A decoded
// icon is a bitmap (a few KiB to tens of KiB); 256 KiB of Base64 decodes to
// ~192 KiB of raw bytes, far beyond any reasonable app icon. Reject larger
// payloads before Base64.decode to bound memory (R4-SCR-01: unbounded
// Base64 icon decode). The remaining attack surface is removed by SCR-01
// (config moved to a root-owned zone); this is defense-in-depth for the
// trusted-RPC config path.
    ...
    try {
        ...
        const items = config.apps.map((obj) => {
            ...
            if (obj.icon_big && obj.icon_big !== '') {
                if (obj.icon_big.length > MAX_ICON_BYTES) {
                    logger.error(
                        `Icon big for ${obj.package} exceeds size cap ` +
                            `(${obj.icon_big.length} > ${MAX_ICON_BYTES}), skipping`
                    );
                } else {
                    try {
            ...
            if (obj.icon_small && obj.icon_small !== '') {
                if (obj.icon_small.length > MAX_ICON_BYTES) {
                    logger.error(
                        `Icon small for ${obj.package} exceeds size cap ` +
                            `(${obj.icon_small.length} > ${MAX_ICON_BYTES}), skipping`
                    );
                } else {
                    try {
                        const bytes = Base64.decode(
                            obj.icon_small,
                            getFieldValue(Base64, 'DEFAULT')
                        );
...
export function runAgent(main) {
    // Skip auto-start when neither Java (Frida runtime) nor rpc (frida-inject
    // session) are present — i.e. plain Node.js, which means the module was
    // imported by a test rather than injected. Previously this guard keyed off
    // `process.env.NODE_ENV === 'test'`, but that is a fragile env-var
    // contract: structural detection (no Java AND no rpc) is unambiguous and
    // does not depend on how the test runner sets env. When Frida loads the
    // script, at least one of Java/rpc is always defined (locally via
    // frida-inject: Java is present even without --parameters; via the
    // voboost-inject daemon: rpc.exports.init is posted before main runs).
    if (typeof Java === 'undefined' && typeof rpc === 'undefined') {
        runAgentLogger.debug(DEBUG.NO_FRIDA_RUNTIME);
        return;
    }

    // Guard against double-start: rpc.exports.init() and the setTimeout
    // fallback can both fire (frida always calls init within the 2s window,
    // but the timeout was never cleared, so startAgent ran twice). See SCR-03.
    let started = false;
    let initTimer = null;

    // Helper function to start the agent (idempotent)
    const startAgent = () => {
        if (started) {
            runAgentLogger.debug('Agent already started, skipping duplicate start');
            return;
        }
        started = true;
        if (initTimer !== null) {
    ...
    if (typeof rpc !== 'undefined') {
        ...
        rpc.exports = {
            init(stage, parameters) {
                ...
                // Start the agent immediately (clears the fallback timeout)
                startAgent();
            },
            dispose() {
        ...
        // Fallback: start the agent if init() is not called within 2s.
        // startAgent() is idempotent and clears this timer when init fires,
        // so this never double-starts the agent. See SCR-03.
