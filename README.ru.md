...
| [`lib/`](lib) | Общие утилиты — [`Logger`](lib/logger.js), [`utils.js`](lib/utils.js), пайплайн сборки Rollup |
...
| [`voboost-to-menu-mod`](agents/voboost-to-menu-mod.js) | launcher | Добавляет пункт меню для запуска приложений |
| [`weather-widget-mod`](agents/weather-widget-mod.js) | launcher | Чинит виджет погоды (требуется ключ OpenWeatherMap API) |

## Система сборки

Проект использует [Rollup](https://rollupjs.org/) для бандлинга каждого агента в
самодостаточный файл в [`build/`](build). Сборка также производит варианты уровней логирования
(`_0none`, `_1error`, `_2info`, `_3debug`), вырезающие код логирования на разных уровнях для
production-настройки. См. [`lib/build.mjs`](lib/build.mjs) и
...
Логирование идёт через класс `Logger` из [`lib/logger.js`](lib/logger.js), который
автоматически интегрируется в каждый скрипт в процессе сборки.
...
Все методы автоматически добавляют временные метки в формате `YYYY-MM-DD HH:mm:ss.SSS`,
совместимые с Kotlin-логгером в приложении `ru.voboost`.
...
Набор включает unit-тесты для экспортируемых helper'ов каждого агента, валидацию сборки
(синтаксис + варианты уровня логов) и соответствие шаблонам логирования. Все тесты должны
проходить перед слиянием.
